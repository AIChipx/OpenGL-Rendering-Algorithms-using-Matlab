function [ frame_out,z_out ] = tri_scan_conv_Z_text_pros( x1, y1, z1, h0, h1, h2, frame_in, Z_buffer,aa,bb,cc,image, GL_TEXTURE_WRAP_S, GL_TEXTURE_WRAP_T, filter_mode )
    ...
    for i=xx_min:xx_max
        ...
        if(Alpha>=0 && Beta>=0 && Gamma >=0)
            ...
            if(depth < Z_buffer(j,i))
                ...
                red_val   = Texture_unit0(text_cord(1),text_cord(2),image(:,:,1), GL_TEXTURE_WRAP_S, GL_TEXTURE_WRAP_T, filter_mode);
                green_val = Texture_unit0(text_cord(1),text_cord(2),image(:,:,2), GL_TEXTURE_WRAP_S, GL_TEXTURE_WRAP_T, filter_mode);
                blue_val  = Texture_unit0(text_cord(1),text_cord(2),image(:,:,3), GL_TEXTURE_WRAP_S, GL_TEXTURE_WRAP_T, filter_mode);
