function [ frame_out,z_out ] = tri_scan_conv_Z_text_pros3( x1, y1, z1,frame_in, Z_buffer,uv1,uv2,uv3,image )
    ...
    for i=xx_min:xx_max
        ...
        if(Alpha>=0 && Beta>=0 && Gamma >=0)
            ...
            if(depth < Z_buffer(j,i))
                ...
                red_val =bi_linear(text_cord(1)/zr,text_cord(2)/zr,image(:,:,1));
                green_val =bi_linear(text_cord(1)/zr,text_cord(2)/zr,image(:,:,2));
                blue_val =bi_linear(text_cord(1)/zr,text_cord(2)/zr,image(:,:,3));
