function [ frame_out,z_out ] = tri_scan_conv_Z_text_pros_LoD( x1, y1, z1, h0, h1, h2, Frame_buffer, Z_buffer,aa,bb,cc,image_1, GL_TEXTURE_WRAP_S, GL_TEXTURE_WRAP_T, GL_TEXTURE_MAG_FILTER, GL_TEXTURE_MIN_FILTER )
        ...
        if(Alpha>=0 && Beta>=0 && Gamma >=0)
            ...
            if(depth < Z_buffer(j,i))
               ...
               % fetching texture color from Texture_unit0
                [red_val  ,] = Texture_unit_LoD_MIP_Tri(image_1(:,:,1), GL_TEXTURE_WRAP_S, GL_TEXTURE_WRAP_T, Grad_Buff, counter_state, GL_TEXTURE_MAG_FILTER, GL_TEXTURE_MIN_FILTER);
                [green_val,] = Texture_unit_LoD_MIP_Tri (image_1(:,:,2), GL_TEXTURE_WRAP_S, GL_TEXTURE_WRAP_T, Grad_Buff, counter_state, GL_TEXTURE_MAG_FILTER, GL_TEXTURE_MIN_FILTER);
                [blue_val ,] = Texture_unit_LoD_MIP_Tri (image_1(:,:,3), GL_TEXTURE_WRAP_S, GL_TEXTURE_WRAP_T, Grad_Buff, counter_state, GL_TEXTURE_MAG_FILTER, GL_TEXTURE_MIN_FILTER);
                
                % updating the frame buffer
                Frame_buffer(j,i,:) = [red_val, green_val, blue_val];
