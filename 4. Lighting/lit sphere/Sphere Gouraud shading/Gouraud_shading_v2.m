% correct value for Z-buffer

% Sphere Gouraud shading
% here we will use lit_viewport_inf
% all normalizations are performed here
% at least 400 vertex required for the specular highlight to appear
% from experiement specular highlight doesn't change position when the
% sphere is rotated
...
%/////////////////////////////////////////////////////////////////////////
% generating correct value to be stored in Z-buffer
% getting size of vertex list
vsize = size(vertex_list);
for p=1:vsize(1)
    t121 =((vl(p,3)- n)/(f - n))*(2^24-1);
    vl(p, 3) = t121;
end
%/////////////////////////////////////////////////////////////////////////
...
% Material properties
    ...
    mat_amb_diff = [0.07568 0.61424 0.07568];
...
% viewport direction (at infinity) 
    ...
    lightsource_dir = normalize(lightsource_dir2);
    % half vector
    h0 = e0 + lightsource_dir;
    h = normalize(h0);
